...
#
# Copyright (C) 2019 Shivam Kumar Jha <carlodandan.personal@proton.me>
#
# Helper functions
...
git -c "user.name=carlodandan" -c "user.email=carlodandan.personal@proton.me" commit -sm "OEM Release" > /dev/null 2>&1
rm -rf ${PROJECT_DIR}/kernels/${UNZIP_DIR}
...
for ELEMENT in ${DIFFPATHS[@]}; do
    ...
    git -c "user.name=carlodandan" -c "user.email=carlodandan.personal@proton.me" commit -sm "Add $ELEMENT modifications" > /dev/null 2>&1
done
# Remaining OEM modifications
git add --all -f > /dev/null 2>&1
git -c "user.name=carlodandan" -c "user.email=carlodandan.personal@proton.me" commit -sm "Add remaining OEM modifications" > /dev/null 2>&1
